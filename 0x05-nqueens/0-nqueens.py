...
def ChessBoard(n: int):
    """This Program solves the N queens problem with
    ...
    def chzBord(row, col, col_in_row):
        """ This is to Checks if queen can be placed without attacking other queens"""
        for r in range(row):
    ...
    def placeQueen(row, cols, col_in_row):
        ...
        for col in range(n):
            if cols[col] == 0 and chzBord(row, col, col_in_row):
                cols[col] = 1
                col_in_row[row] = col
...
if __name__ == '__main__':
    ...
    nqueens = chzBord(int(sys.argv[1]))
